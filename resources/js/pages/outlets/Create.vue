...
import { index, store } from '@/routes/outlets';
...
interface CreateOutletPageProps {
    tenants?: Tenant[];
    isSuperAdmin: boolean;
}

const props = defineProps<CreateOutletPageProps>();
...
const form = useForm({
    ...
    name: '',
    address: '',
    mode: 'pos' as 'pos' | 'restaurant' | 'minimarket',
    settings: {},
});
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 px-4">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-semibold">Create Outlet</h1>
                    <p class="text-sm text-muted-foreground">
                        {{ isSuperAdmin ? 'Add a new outlet to any tenant' : 'Add a new outlet to your business' }}
                    </p>
                </div>
            </div>
            ...
            <Card class="max-w-2xl">
                <CardHeader>
                    ...
                    <CardDescription>
                        Enter the details for your new outlet
                    </CardDescription>
                </CardHeader>
                <CardContent class="space-y-6">
                    <form @submit.prevent="submit" class="space-y-6">
                        <!-- Tenant Selection for Superadmins -->
                        <div v-if="isSuperAdmin" class="space-y-2">
                            ...
                            <p v-if="form.errors.tenant_id" class="text-sm text-red-600">
                                {{ form.errors.tenant_id }}
                            </p>
                            <p class="text-xs text-muted-foreground">
                                Choose which tenant this outlet belongs to
                            </p>
                        </div>
